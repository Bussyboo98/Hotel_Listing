...
def hotel(request):
    ...
    paginated_filter = Paginator(all_post, 8)
...
def blog(request):
    ...
    paginated_filter = Paginator(blg_post, 5)
    page_number = request.GET.get('page')
    person_page_obj = paginated_filter.get_page(page_number)
    context = {
...
def contact(request):
    if request.method == 'POST':
        ...
        send = mail.send_mail(subject, plain_message, from_email, [
                    'ogunburebusayo.j@gmail.com', 'tijaniyunus07@gmail.com', email], html_message=html_message, fail_silently=True)
        if send:
            messages.success(request, 'Email sent, you will recieve an email shortly!')
        else:
